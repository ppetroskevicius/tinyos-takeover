...
NFS_HOST="192.168.20.33"

# list of hosts to check for updates
UPDATE_URLS="http://192.168.20.33:2543/takeover.internal.sh https://raw.githubusercontent.com/ppetroskevicius/tinyos-takeover/main/apkovl/opt/tinybox/takeover.sh"
# list of hosts to fetch the image from
IMG_HOSTS="http://192.168.20.33:2543"
...
EXPECTED_MEMORY_SIZE_GB=64 #128
EXPECTED_CORE_COUNT=32
EXPECTED_DRIVE_COUNT=5 #4
EXPECTED_DRIVE_LINK_SPEED="16GT/s"
EXPECTED_DRIVE_LINK_WIDTH="x4"

echo "text,System Check.. ,System Check ..,System Check. ." | nc -U /run/tinybox-screen.sock 2> /dev/null
...
for busid in $gpu_busids; do
  echo "text,Checking GPU $i,$busid" | nc -U /run/tinybox-screen.sock 2> /dev/null
...
echo "text,Found $drive_count Drives" | nc -U /run/tinybox-screen.sock 2> /dev/null
...
# for machines with < 128GB ram mount the nfs share to store the image
apk update
apk add nfs-utils
mkdir -p /tmp/tmp
mount -t nfs "$NFS_HOST:/raid/nfs/share /tmp/tmp"
...
for host in $IMG_HOSTS; do
  if wget -q --spider "$host/tinyos.red.img"; then
    selected_host="$host"
    break
  fi
done
...
if [ -z "$selected_host" ]; then
  echo "text,No Host Found" | nc -U /run/tinybox-screen.sock 2> /dev/null
  exit 1
else
  echo "text,Using Host,$selected_host" | nc -U /run/tinybox-screen.sock 2> /dev/null
fi
...
if ! grep -q "100%" /tmp/log; then
  echo "text,Download Failed" | nc -U /run/tinybox-screen.sock 2> /dev/null
  exit 1
fi
